...
V_comfirmedPdeaths <- c() #Will Hold all 289 entries of the sum of deaths_global+confirmed_global
...
for(j in 1:jCntLength) #Iterate through rows 1-289  
  {
    ...
    #Populate vectors
    V_comfirmedPdeaths <- c(V_comfirmedPdeaths, DCtot) #adds deaths_global + confirmed_global to new vector
    V_Bloc <- c(V_Bloc, Bloc) #adds Country/Region to new vector
  }

#for loop consolidates all multiple country data rows

Final_comfirmedPdeaths <- c() #Will Hold all 289 entries of the sum of deaths_global+confirmed_global
...
for(j in 1:jCntLength) #Iterate through rows 1-289  
  ...
  if(V_Bloc[j] == V_Bloc[1])
    { 
    Final_comfirmedPdeaths <- c(Final_comfirmedPdeaths, V_comfirmedPdeaths[j]) #adds deaths_global + confirmed_global to new vector
    Final_Bloc <- c(Final_Bloc, deaths_global[j,2]) #adds Country/Region to new vector
    }
  else if(V_Bloc[j] %in% V_Bloc[j+1])
    { 
    ...
    { 
    sumCnD <- sumCnD + V_comfirmedPdeaths[j]
    Final_comfirmedPdeaths <- c(Final_comfirmedPdeaths, sumCnD) #adds deaths_global + confirmed_global to new vector
    Final_Bloc <- c(Final_Bloc, deaths_global[j,2]) #adds Country/Region to new vector
    sumCnD <- 0
    }
  else
...
df_decreasing <- df_comfirmedPdeaths[order(df_comfirmedPdeaths$Final_comfirmedPdeaths, decreasing = TRUE),]
top5 <- df_decreasing[-(6:RowCut),]
knitr::kable(top5, "pipe", col.name=c('Country/Region','comfirmed & deaths'),caption = "TOP 5: Recent Comfirmations & Deaths",align=c("r","c","c"))


df_increasing <- df_comfirmedPdeaths[order(df_comfirmedPdeaths$Final_comfirmedPdeaths, decreasing = FALSE),]
bot5 <- df_increasing[-(6:RowCut),]
