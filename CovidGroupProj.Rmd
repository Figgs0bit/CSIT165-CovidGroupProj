...
for(j in 1:jCntLength) #Iterate through rows 1-289  
{
  ...
  if(V_Bloc[j] == V_Bloc[1]) #This if conditional is need as the the first row will not fall through the remaing if statements.
  { 
...
RowCut <- nrow(df_comfirmedPdeaths) #this creates a variable need to cut/delete unneeded rows. 

df_decreasing <- df_comfirmedPdeaths[order(df_comfirmedPdeaths$Final_comfirmedPdeaths, decreasing = TRUE),] #list the Dataframe in decreasing order
...
df_increasing <- df_comfirmedPdeaths[order(df_comfirmedPdeaths$Final_comfirmedPdeaths, decreasing = FALSE),] #list the Dataframe in increasing order
