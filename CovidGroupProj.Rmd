...
max_value <- as.numeric(ncol(confirmed_global[,]) * nrow(confirmed_global[,])) #serves as while loop limit 331,483.
...
#Variables for highest and lowest Risk score        
low_value <- 1
#Holds data frame coordinates for low
iLow <- 1 
jLow <- 1

high_value <- 0
#Holds data frame coordinates for hi
iHi <- 1 
jHi <- 1

#For loop iterates throught the entire data frame
for(j in 1:jCntLength) #Iterate through rows 1-289 
  {
  for(i in 5:iCntLength) #Iterate through columns 1-1147
    {
      emptyClr <- (deaths_global[j,3] + deaths_global[j,4]) #This variable serves to help filter out locations with zero coordinates.

      if(emptyClr > 0 && confirmed_global[j,i] != 0) #Filters out the first 4 Columns of empty cells in province.state 
        {
        #Global riskscore
        SUMdeaths_global <- SUMdeaths_global + deaths_global[j,i] #holds the sum of every death in data frame.
        SUMconfirmed_global <- SUMconfirmed_global + confirmed_global[j,i] #holds the sum of every confirmation in data frame.
        riskScore <- (deaths_global[j,iCntLength]/confirmed_global[j, iCntLength])*100 #Riskscore eqn.=(deaths/confirmations)x100

          #Country with lowest risk score (if more than one, display the one with the most confirmations)
          if (riskScore > high_value) 
            {
            high_value <- riskScore
            #saves the (i,j) row and col coordinates for high value
            jHi <- j 
            iHi <- i
            }
          ...
          if (riskScore < low_value)
            {
            low_value <- riskScore
            #saves the (i,j) row and col coordinates for low value
            jLow <- j
            iLow <- i
            }      
        }
...
sumRiskScore <- (SUMdeaths_global/SUMconfirmed_global)*100 #Average Risk Score equation
cat("\nAverage Risk Score:",sumRiskScore, "\n\n")

cat("Row positions: ",jHi,",Column Position:",iHi,"\n")
cat("Highest current Risk Score:", high_value,"\n")
cat("On Date",ColName[i],",Deaths",deaths_global[jHi,iHi],"/confirmed",confirmed_global[jHi,iHi],"\n")
...
cat("Row positions: ",jLow,",Column Position:", iLow,"\n")
cat("Lowest current Risk Score:", low_value,"\n")
cat("On Date",ColName[i],"Deaths",deaths_global[jLow,iLow],"/confirmed",confirmed_global[jLow,iLow],"\n")
...
#### Objective 4.2
```{r ob4.2}
#Country with highest risk score (if more than one, display the one with the most confirmations)
...
for(k in 1:jCntLength) #Iterate through rows and serves as for loop limit 289
  {
  #If else statement filters out zero confirmed_global values.
  #The confirmed_global variable serves as denominator and can never equal zero. 
  if(confirmed_global[k, jCntLength] == 0) 
    {chkRiskScore <- 0}
  else
    {chkRiskScore <- (deaths_global[k,iCntLength]/confirmed_global[k, iCntLength])*100}

  
  if(chkRiskScore == high_value)  
    {     
      if(confirmed_global[k, iCntLength] >= HiMaxConfirmed)
        {
        HiMaxConfirmed <- confirmed_global[k, iCntLength] 
        HiColNum <- k
        }
    }
  ...
  if(chkRiskScore == low_value)  
    {     
      if(confirmed_global[k, iCntLength] < LoMaxConfirmed)
        {
        LoMaxConfirmed <- confirmed_global[k, iCntLength] 
        LoColNum <- k
        }
    }
  }
